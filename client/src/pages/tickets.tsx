  ...
  return (
    <div className="flex-1 flex flex-col pb-20">
      <header className="px-4 pt-12 pb-4 bg-neutral-900 sticky top-0 z-10">
        <h1 className="font-display font-bold text-2xl text-white mb-4">My Tickets</h1>
          ...
          <TabsContent value="upcoming">
            <div className="mt-4">
              {ticketsLoading ? (
                ...
                tickets
                  .filter(ticket => new Date(ticket.purchaseDate) > new Date())
                  .map(ticket => (
                    <TicketCard 
                      key={ticket.id} 
                      ticket={ticket} 
                      onClick={() => navigate(`/ticket/confirmation/${ticket.referenceNumber}`)} 
                    />
                  ))
              ) : (
